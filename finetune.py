...
from model import Resnet50_Module, Densenet121_Module, SwinB_Module, SwinT_Module, ConvNeXtB_Module

from data_handler import DataHandler_CovidX_Finetuning
from training_args import add_finetune_args
...
def main(gpu, ngpus_per_node, args):
    ...
    elif (args.arch == 'convnext-b'):
        model = ConvNeXtB_Module(
            args, 
            verbose=True,
            imratio=imratio,
        )   
    ...
    if (localmaster):
        ...
        for model_name in model_names:
            save_dir = os.path.join(args.exp_dir, 'metrics', model_name)
            utils.prepare_directory(save_dir)
            model.evaluate(test_dataloader, os.path.join(args.exp_dir, model_name + '.ckpt'), save_dir)
